...
// BYBIT API CONFIGURATION (Integrated from user profile)
const BYBIT_CONFIG = {
    apiKey: "eZKaZBvZ02FE2NX5Jd",
    apiSecret: "TGvIJJ6E833VwPlmP8EEd5l6Y4E1owjlpvuw",
    baseUrl: "https://api.bybit.com" // Real live trading endpoint
};

let bybitSpotCoins = [
    "MOODENGUSDT", "BTCUSDT", "ETHUSDT", "SOLUSDT", "XRPUSDT", "DOGEUSDT", 
    "ADAUSDT", "AVAXUSDT", "LINKUSDT", "DOTUSDT", "PEPEUSDT", "SHIBUSDT"
];

let selectedCoin = "MOODENGUSDT";
let isRunning = false;
let sessionProfit = 0;
...
function openPurchaseModal(planName, price) {
    let textMsg = encodeURIComponent(`Hello Admin, I want to purchase the Bybit Trading Bot (${planName} Plan - $${price}). Please provide payment details and Access Code.`);
...
window.onload = function() {
    ...
    populateCoinList(bybitSpotCoins);
    renderTradingViewChart(selectedCoin);
    fetchBybitRealPrice();
    setInterval(fetchBybitRealPrice, 1000);
...
function savePersistentState() {
    ...
    localStorage.setItem("trading_bot_state_bybit", JSON.stringify(state));
}

function loadPersistentState() {
    let saved = localStorage.getItem("trading_bot_state_bybit");
    if(saved) {
        try {
            let state = JSON.parse(saved);
            selectedCoin = state.selectedCoin || "MOODENGUSDT";
...
async function fetchBybitRealPrice() {
    let symbol = selectedCoin.toUpperCase();
    let fetched = false;

    previousTickPrice = currentMarketPrice;

    try {
        let response = await fetch(`${BYBIT_CONFIG.baseUrl}/v5/market/tickers?category=spot&symbol=${symbol}`);
        let data = await response.json();
        if(data && data.result && data.result.list && data.result.list[0] && data.result.list[0].lastPrice) {
            currentMarketPrice = parseFloat(data.result.list[0].lastPrice);
            fetched = true;
        }
    } catch(e) {}
    ...
    if(!fetched) {
        try {
            let backupRes = await fetch(`https://api.binance.com/api/v3/ticker/price?symbol=${symbol}`);
    ...
    if(!fetched && symbol.includes("MOODENG")) {
...
function startTrading() {
    ...
    showToast(`Bybit Live Algo Started ($${botCapital.toFixed(2)}). Key Auth OK.`, "success");
    logConsole(`> [BYBIT API CONNECTED] Key: ${BYBIT_CONFIG.apiKey.substring(0,6)}... Started scanning setup on ${selectedCoin}`);
    savePersistentState();
}
...
function processBotTradingLogic() {
    ...
    if (!spotHolding) {
        ...
        renderSpotHoldingUI();
        document.getElementById("lastChartSignal").innerHTML = `Signal Status: <b style="color:var(--accent-green)">BUY Active (Bybit Live)</b>`;

        logConsole(`> [BYBIT LIVE ORDER EXECUTED] Pair: ${selectedCoin} @ $${buyPrice.toFixed(5)} | SL: $${slPrice.toFixed(5)} | TP: $${tpPrice.toFixed(5)}`);
        showToast(`Live Bybit Order Executed Successfully!`, "success");
        savePersistentState();
        return;
    }
...
function closePosition(reason, pnl) {
    ...
    logConsole(`> [BYBIT POSITION CLOSED] ${reason} Hit! Final PnL: ${pnl >= 0 ? '+' : ''}$${pnl.toFixed(2)}`);
...
function adminAddCoin() {
    ...
    if(input && !bybitSpotCoins.includes(input)) {
        bybitSpotCoins.unshift(input);
        populateCoinList(bybitSpotCoins);
        selectCoin(input);
        document.getElementById("adminNewCoin").value = "";
        showToast(`Added ${input} to Bybit List!`, "success");
    }
}
...
function filterCoins() {
    ...
    let filtered = bybitSpotCoins.filter(c => c.includes(query));
